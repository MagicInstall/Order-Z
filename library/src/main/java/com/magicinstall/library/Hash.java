...
public class Hash {
     ...
     * @return
     */
    public static int byte32ToInt(@NonNull byte val[], int offset, int length) {
        int result = 0;
        if (length > 3 && val.length > (offset + 3)) {
            result |= val[offset + 3] & 0xFF;
            result <<= 8;
        }
        if (length > 2 && val.length > (offset + 2)) {
            result |= val[offset + 2] & 0xFF;
            result <<= 8;
        }
        if (length > 1 && val.length > (offset + 1)) {
            result |= val[offset + 1] & 0xFF;
            result <<=  8;
        }
        if (length > 0 && val.length > (offset + 0)) {
    ...
    public static int byte32ToInt(@NonNull byte val[], int offset) {
        return byte32ToInt(val, offset, 4);
    }
    public static int byte32ToInt(@NonNull byte val[]) {
        return byte32ToInt(val, 0, 4);
    }
